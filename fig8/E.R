...
setwd('D:/R/ML_PAAD/data1/AUCAtcga/DEGs&WGCNA/TPM_0.3_0.01_1_sec/0.6_0.2/down/linkET_CIB')
...
CIBER <- CIBER[-1,]
CIBER <- data.frame(rownames(CIBER),CIBER)
...
pdf(paste('linkET_CIB_all.pdf',sep=''),width=length(colnames(CIBER))/2.6,height=length(colnames(CIBER))/2.6)
...
pdf(paste('linkET_CIB_hot_low.pdf',sep=''),width=length(colnames(CIBER))/2.6,height=length(colnames(CIBER))/2.6)
...
pdf(paste('linkET_CIB_cold_high.pdf',sep=''),width=length(colnames(CIBER))/2.6,height=length(colnames(CIBER))/2.6)
qcorrplot(correlate(out), type = "lower", diag = FALSE) +#热图绘制
  geom_square() +#热图绘制
  geom_couple(aes(colour = pd, size = rd),data = mantel,curvature = nice_curvature()) +#aes里面是线条格式，data对应的是mantel test 计算结果，curvature控制线条曲率
  scale_fill_gradientn(colours = rev(RColorBrewer::brewer.pal(11, "RdBu")), limits = c(-0.5, 0.5)) +
